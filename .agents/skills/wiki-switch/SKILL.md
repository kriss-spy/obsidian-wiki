...
description: >
  ...
  The skill manages named config files at ~/.config/obsidian-wiki/config.NAME and activates one by
  symlinking it to ~/.config/obsidian-wiki/config.
---

# Wiki Switch — Manage Multiple Vault Profiles

Each vault is a complete config file at `~/.config/obsidian-wiki/config.<name>`. The active vault is
whichever file `~/.config/obsidian-wiki/config` symlinks to. Switching vaults means re-pointing that symlink.
...
1. Verify `~/.config/obsidian-wiki/config.<name>` exists. If not, tell the user the vault doesn't exist and list what's available (run **List**).
2. Run:
   ```bash
   mkdir -p ~/.config/obsidian-wiki
   ln -sf ~/.config/obsidian-wiki/config.<name> ~/.config/obsidian-wiki/config
   ```
...
1. Find all files matching `~/.config/obsidian-wiki/config.*` (exclude `config` itself — that's the symlink).
2. Resolve the current symlink target: `readlink ~/.config/obsidian-wiki/config`
...
- If a name is given, read `~/.config/obsidian-wiki/config.<name>`.
- If no name given, read `~/.config/obsidian-wiki/config` (the active vault).
...
1. Check `~/.config/obsidian-wiki/config.<name>` doesn't already exist. Abort if it does.
2. Copy the active config:
   ```bash
   mkdir -p ~/.config/obsidian-wiki
   cp ~/.config/obsidian-wiki/config ~/.config/obsidian-wiki/config.<name>
   ```
...
5. Write the updated values into `~/.config/obsidian-wiki/config.<name>`.
6. Update the top comment line to describe the new vault (e.g., `# Obsidian Wiki — <name> vault`).
7. Confirm:
   ```
   Created: ~/.config/obsidian-wiki/config.<name>
